...
//output the default board as a pointer
void set_up(Board* board, int row_size, int col_size){
	*board = make_board(row_size, col_size);
}
//Defalut board made with number of rows and colunms
...
void modify_hori_board(Board board, Board* new_board, int row, int col1, int col2){
	int i;
	*new_board = board;
	//recognize the input from two directions
	if(col1>col2){
		for(i=col2;i<=col1;++i){
...
void modify_row_board(Board board, Board* new_board, int row){
	int i, j;
	//add a row at specified position
	*new_board = board;
...
void modify_col_board(Board board, Board* new_board, int col){
	int i, j;
	*new_board = board;
	//add a colunm at specified position
...
void resize_board(Board board, Board* new_board, int num_rows, int num_cols){
	int i, j;
	*new_board = board;
	//compare the inputs with current number of rows and colunms to decide which part to realloc or malloc
